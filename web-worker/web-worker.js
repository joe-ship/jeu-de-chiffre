...
//élément indiquant le temps total de réflexion
var jeuTemps = document.getElementById("jeuTemps"); 
//élément indiquant le temps restant pour jouer
var jeuCible = document.getElementById("jeuCible");  
//élément indiquant le nombre à trouver
...
var operation = this.value.match(/(\d+)\s*([-+*_\\/÷&xX×])\s*(\d+)/); 
//permet de vérifier que l'opération contient un nombre, un opérateur, et un nombre
if(operation){
...
var distance = Infinity; //distance de la solution actuelle par rapport à la
cible
...
rslt = chercheSolution(nombresSansNb2.concat([calcul]),cible); // on Partie 1 : La base des web-workers 23/86relance la recherche avec les nombres restant + ce résultat
if(rslt[0]<distance){
distance = rslt[0];
solution = nb1 +" + " + nb2 + " = " + calcul + "\n" + rslt[1];
if(!distance) break; //on a trouvé une solution on arrête la boucle
